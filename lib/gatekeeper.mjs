  ...
  if (requirements.length === 0) {
    ...
    if (requirements.length === 0) {
      for (const raw of lines) {
        ...
        if (HEADING_RE.test(trimmed) || BOLD_HEADING_RE.test(trimmed)) continue;
        const line = stripInline(trimmed);
 ...
 * matches the same keyword in the resume; internal tech punctuation is
...
export function matchRequirement(reqText, corpusLower, cvLines) {
  ...
  if (present.length) {
    ...
    if (hit) {
      const clean = stripInline(hit).replace(/^(?:#{1,6}|[-*])\s+/, '');
...
export function buildGatekeeperScaffold(jdText, cvText, digestText = '') {
  ...
  let gapsBody;
  if (gaps.length) gapsBody = gaps.map((g) => `- ${g}`).join('\n');
  else if (rows.length === 0) gapsBody = 'None: no requirements were extracted from the JD.';
  else gapsBody = 'None: every extracted requirement had at least partial keyword coverage.';
  blocks.push(gapsBody);
